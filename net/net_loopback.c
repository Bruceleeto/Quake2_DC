...
int NET_Socket (char *net_interface, int port)
{
    ...
    // make it non-blocking using fcntl
    flags = fcntl(newsocket, F_GETFL, 0);
    if (flags == -1)
    {
        Com_Printf ("ERROR: UDP_OpenSocket: fcntl get flags: %s\n", NET_ErrorString());
        close(newsocket);
        return 0;
    }
    ...
    {
        Com_Printf ("ERROR: UDP_OpenSocket: fcntl set nonblock: %s\n", NET_ErrorString());
        close(newsocket);
        return 0;
    }

    // Skip SO_BROADCAST setup since it's not supported
