...
void FS_AddGameDirectory (char *dir)
{
    ...
    strcpy(search->filename, base_dir);
    ...
    for (i = 0; i < 10; i++)
    {
        Com_sprintf(pakfile, sizeof(pakfile), "%s/pak%i.pak", base_dir, i);
...
void FS_InitFilesystem (void)
{
    ...
    // basedir <path>
    // Set this to "." to look in the current directory
    fs_basedir = Cvar_Get ("basedir", ".", CVAR_NOSET);

    // cddir <path>
    fs_cddir = Cvar_Get ("cddir", "", CVAR_NOSET);
    if (fs_cddir->string[0])
        FS_AddGameDirectory (fs_cddir->string);

    // Add the base game directory
